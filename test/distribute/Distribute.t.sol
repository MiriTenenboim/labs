...
contract DistributeTest is Test {
    ...
    function testDistributeTotalAmount() public {
        uint256 amount = 300;

        vm.deal(address(distribute), amount);

        distribute.distributeTotalAmount(amount);
        assertEq(usersAddresses[0].balance, amount / usersAddresses.length);
    }
